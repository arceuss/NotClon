...
void copyImportAssets(const std::string& smPath, const std::string& outDir,
                      const std::vector<std::string>& refs,
                      SmImportReport& report) {
    namespace fs = std::filesystem;
    std::error_code ec;
    const fs::path sourceDir = fs::absolute(fs::path(smPath).parent_path(), ec);
    if (ec) return;
    const fs::path destination = fs::absolute(outDir, ec);
    if (ec) return;
    const bool sameDirectory = fs::equivalent(sourceDir, destination, ec);
    if ((!ec && sameDirectory) ||
        sourceDir.lexically_normal() == destination.lexically_normal()) return;
    ec.clear();

    std::set<std::string> copied;
    auto copyFile = [&](const fs::path& source, const fs::path& target) {
        ...
        if (!copyError)
            fs::copy_file(source, target, fs::copy_options::overwrite_existing,
                          copyError);
        if (copyError)
            report.warnings.push_back("could not copy '" +
    ...
    for (std::string ref : refs) {
        ref = trim(ref);
        if (ref.empty() || ref[0] == '-' || ref[0] == '@') continue;
        const fs::path relative = fs::path(ref).lexically_normal();
        if (relative.empty() || relative == "." || relative.is_absolute()) {
            report.warnings.push_back("asset path '" + ref +
                                      "' is not relative to the song folder -- skipped");
            continue;
        }
        bool escapes = false;
        ...
        if (escapes) {
            report.warnings.push_back("asset path '" + ref +
                                      "' leaves the song folder -- skipped");
            continue;
        }
        const std::string key = relative.generic_string();
        if (!copied.insert(key).second) continue;

        const fs::path source = sourceDir / relative;
        const fs::path target = destination / relative;
        if (fs::is_directory(source, ec)) {
            ec.clear();
            fs::create_directories(target, ec);
            if (ec) {
                report.warnings.push_back("could not create '" +
            ...
            while (!ec && it != end) {
                ...
                const fs::path childRelative = fs::relative(child, source, ec);
                if (ec) break;
                const fs::path childTarget = target / childRelative;
                if (it->is_directory(ec)) fs::create_directories(childTarget, ec);
                else if (it->is_regular_file(ec)) copyFile(child, childTarget);
                if (ec) break;
                it.increment(ec);
            }
            if (ec)
                report.warnings.push_back("could not copy actor folder '" + ref +
                                          "': " + ec.message());
        } else if (fs::is_regular_file(source, ec)) {
            copyFile(source, target);
        } else {
            report.warnings.push_back("referenced asset '" + ref +
                                      "' was not found beside the .sm");
        }
        ec.clear();
    }
}
...
// One OITG mod string -> ModDoc entries at `tick`, live for `len` ticks.
//
// Extracted from importSm so the Lua actor bridge can share it: gat expresses
// its whole modchart as `mod_insert(beat, len, '*10 50 flip', ...)`, which is
// the same `*<approach> <percent> <name>` grammar #MODS/#ATTACKS uses. Two
// parsers for one grammar would drift, and the drift would be silent.
void addModString(ModDoc& doc, const std::string& modsField,
    ...
    for (const std::string& tokRaw : split(modsField, ',')) {
        ...
        // PlayerOptions::FromOneModString (SM5.1 :577-626, OITG the same
        // shape). The token is split on WHITESPACE and every word is examined
        // independently:
        //
        //     "no"      -> level 0
        //     digit|'-' -> level = StringToFloat(word)/100
        //     '*'       -> approach speed
        //
        // and then `sBit = asParts.back()` -- THE LAST WORD IS THE NAME. Order
        // is not fixed and earlier words are not the name.
        //
        // Two things this gets right that a fixed-order scan does not:
        //  * "*10 flip *10 reverse" is a malformed row a real modchart
        //    contains. SM reads it as `*10 reverse` and silently drops the
        //    `flip`. Treating the remainder as one name invented a mod called
        //    "flip *10 reverse".
        //  * "1.2246467991474e-14 square" -- a value a sin() schedule produces
        //    at its endpoint. StringToFloat takes scientific notation, so a
        //    numeric test that rejects 'e' leaves the number glued to the name.
        ...
        std::string name;
        {
            ...
            for (const std::string& w : words) {
                if (lower(w) == "no") { percent = 0.0f; continue; }
                if (w[0] == '*') { approach = float(atof(w.c_str() + 1)); continue; }
                // strtod, not a hand-rolled digit scan: it accepts the
                // exponent form and tells us how much it consumed, so a word
                // is numeric only if it is numeric ALL THROUGH.
                if (isdigit((unsigned char)w[0]) || w[0] == '-' || w[0] == '+' ||
                    w[0] == '.') {
                    char* endp = nullptr;
                    const double v = strtod(w.c_str(), &endp);
                    if (endp && *endp == '\0') { percent = float(v) / 100.0f; continue; }
                    if (endp && *endp == '%' && endp[1] == '\0') {
                        percent = float(v) / 100.0f; continue;
                    }
                }
            }
            name = lower(words.back());   // sBit = asParts.back()
        }
        if (name.empty()) continue;
        std::string tokName = name;
        ...
        if (id < 0) {
            if (std::find(st.unknown.begin(), st.unknown.end(), name) == st.unknown.end())
                st.unknown.push_back(name);
            continue;
        }
        if (modIsStub(id) &&
            std::find(st.stubbed.begin(), st.stubbed.end(), name) == st.stubbed.end())
            st.stubbed.push_back(name);

        doc.entries.push_back(ModEntry{tick, id, percent, approach, len, 0, true});
        ++st.entries;
        if (len > 0) ++st.intervals;
    }
}
...
SmImportReport importSm(const std::string& smPath, const std::string& outDir,
    ...
    std::vector<std::string> assetRefs = {
        std::filesystem::path(smPath).filename().string()
    };
    ...
    for (const MsdValue& v : vals) {
        ...
        if      (tag == "TITLE")  R.title  = trim(p1);
            ...
            R.music = trim(p1);
            assetRefs.push_back(R.music);
        }
        else if (tag == "BANNER" || tag == "BACKGROUND" ||
                 tag == "CDTITLE" || tag == "JACKET" ||
                 tag == "CDIMAGE" || tag == "DISC" || tag == "LYRICSPATH")
            assetRefs.push_back(trim(p1));
        else if (tag == "KEYSOUNDS") {
            for (const std::string& file : split(p1, ','))
                assetRefs.push_back(trim(file));
        }
        ...
        } else if (tag == "FGCHANGES" || tag == "BGCHANGES" ||
                   tag == "ANIMATIONS") {
            std::string joined;
            for (size_t i = 1; i < v.params.size(); ++i) {
                if (i > 1) joined += ":";
            ...
            for (const std::string& expr : split(joined, ',')) {
                SmBgChange change;
                if (!smBgDecode(expr, change)) continue;
                assetRefs.push_back(trim(change.file1));
                assetRefs.push_back(trim(change.file2));
                if (tag == "FGCHANGES" && R.fgActor.empty())
                    R.fgActor = trim(change.file1);
            }
    ...
    std::filesystem::create_directories(outDir, outputError);
    if (outputError) {
        R.error = "cannot create " + outDir + ": " + outputError.message();
        return R;
    }
    ...
    copyImportAssets(smPath, outDir, assetRefs, R);
    ...
    for (const std::string& line : modLines) {
        ...
        ModStringStats st;
        st.unknown.swap(unknown);
        st.stubbed.swap(R.stubbed);
        addModString(doc, modsField, tick, len, st);
        unknown.swap(st.unknown);
        R.stubbed.swap(st.stubbed);
        R.modEntries += st.entries;
        R.modIntervals += st.intervals;
    }
